...
var mouse_dir_2d: Vector2 = Vector2(0, -1)

# --- Climbing ---
# The player can climb onto a higher surface that is within step-up reach and has
# room to stand, by pushing into it and pressing the climb key. Higher walls are
# reached by chaining climbs off intermediate surfaces. See _find_climb_target.
var is_climbing: bool = false
# Distance from the body origin down to the feet (collision box bottom). The origin
# sits at head height, so probes must be taken low to catch short obstacles.
const CLIMB_FOOT_OFFSET: float = 0.6
const CLIMB_MAX_HEIGHT: float = 1.1  # Highest surface (above the feet) that can be climbed in one step
const CLIMB_PROBE_DIST: float = 0.7  # How far ahead to look for an obstacle
const CLIMB_STAMINA_COST: float = 15.0  # Base stamina spent per climb
const CLIMB_STAMINA_MIN: float = 5.0  # Need at least this much stamina to climb
# Obstacles to detect/climb: static obstacles (3), containers (7), transparent furniture (8)
const CLIMB_OBSTACLE_MASK: int = (1 << 2) | (1 << 6) | (1 << 7)
...
func _connect_signals():
	...
	PlayerInputSignalBroker.climb().connect(_on_climb)
...
func _physics_process(delta: float) -> void:
	...
	# While climbing, a tween drives global_position; skip gravity/movement entirely.
	if is_climbing:
		return
	...
	if is_alive:
		...
		if not knockback_active:
			...
			var facing_dir = Vector3(mouse_dir_2d.x, 0, mouse_dir_2d.y).normalized()
			if facing_dir.length_squared() < 0.1:
				facing_dir = Vector3(0, 0, -1)
			var right_dir = Vector3(-facing_dir.z, 0, facing_dir.x)
			var direction = (facing_dir * -move_input.y + right_dir * move_input.x).normalized()
			# Athletics skill level
			var athletics_level = get_skill_level("athletics")

			# Calculate run multiplier and stamina modifications
			run_multiplier = 1.5 + (athletics_level / 100.0) * (2.5 - 1.5)
			...
			# Stamina regeneration when standing still or walking
			var horizontal_speed = Vector2(velocity.x, velocity.z).length()
			if horizontal_speed < 0.1:
				current_stamina += delta * stamina_regen_while_standing_still
			elif not is_running or current_stamina == 0:
				current_stamina += delta * (stamina_regen_while_standing_still * 0.5)
...
# The climb key was pressed. If the player is pushing into a climbable obstacle and
# has stamina, climb onto it.
func _on_climb() -> void:
	if is_climbing or not is_alive or is_stunned() or knockback_active or pushing_furniture:
		return
	if current_stamina < CLIMB_STAMINA_MIN:
		return
	if move_input.length() < 0.1:
		return  # You climb the thing you are walking into
	var target: Dictionary = _find_climb_target()
	if target.get("valid", false):
		_execute_climb(target["destination"])


# Works out whether there is a surface ahead the player can climb onto. Returns
# {"valid": true, "destination": Vector3} when a reachable, unobstructed ledge is
# found in the movement direction, otherwise {"valid": false}.
func _find_climb_target() -> Dictionary:
	var invalid: Dictionary = {"valid": false}
	var facing_dir = Vector3(mouse_dir_2d.x, 0, mouse_dir_2d.y).normalized()
	if facing_dir.length_squared() < 0.1:
		facing_dir = Vector3(0, 0, -1)
	var right_dir = Vector3(-facing_dir.z, 0, facing_dir.x)
	var climb_dir = (facing_dir * -move_input.y + right_dir * move_input.x).normalized()
	if climb_dir.length() < 0.1:
		return invalid
	climb_dir = climb_dir.normalized()

	var foot_y: float = global_position.y - CLIMB_FOOT_OFFSET

	# 1. There must be an obstacle directly ahead. Probe low (near the feet) so short
	# obstacles like fences and crates register.
	var probe_from: Vector3 = Vector3(global_position.x, foot_y + 0.2, global_position.z)
	var probe_to: Vector3 = probe_from + climb_dir * CLIMB_PROBE_DIST
	var obstacle: Dictionary = Helper.raycast(probe_from, probe_to, CLIMB_OBSTACLE_MASK, [self])
	if obstacle.is_empty():
		return invalid

	# 2. Find the top surface of that obstacle. Try multiple depths to support narrow
	# objects pushed against walls (like stoves).
	for depth in [0.3, 0.2, 0.1]:
		var dest_xz: Vector3 = obstacle.position + climb_dir * depth
		var surface: Dictionary = _probe_surface_y(dest_xz, foot_y)
		if not surface.found:
			continue

		# 3. The surface must be higher than the feet but within step-up reach.
		var height_gain: float = surface.y - foot_y
		if height_gain <= 0.1 or height_gain > CLIMB_MAX_HEIGHT:
			continue

		# 4. There must be room for the player to stand on top.
		if _climb_blocked(dest_xz.x, dest_xz.z, surface.y):
			continue

		var destination: Vector3 = Vector3(dest_xz.x, surface.y + CLIMB_FOOT_OFFSET, dest_xz.z)
		return {"valid": true, "destination": destination}

	return invalid


# Casts straight down at the given XZ to find the top surface height there.
func _probe_surface_y(probe_xz: Vector3, foot_y: float) -> Dictionary:
	var start: Vector3 = Vector3(probe_xz.x, foot_y + CLIMB_MAX_HEIGHT + 0.5, probe_xz.z)
	var end: Vector3 = Vector3(probe_xz.x, foot_y - 1.0, probe_xz.z)
	var hit: Dictionary = Helper.raycast(start, end, CLIMB_OBSTACLE_MASK, [self])
	if hit.is_empty():
		return {"found": false, "y": 0.0}
	return {"found": true, "y": hit.position.y}


# True if something occupies the space the player would stand in after climbing.
func _climb_blocked(x: float, z: float, surface_y: float) -> bool:
	var from: Vector3 = Vector3(x, surface_y + 0.15, z)
	var to: Vector3 = Vector3(x, surface_y + 0.75, z)
	var hit: Dictionary = Helper.raycast(from, to, CLIMB_OBSTACLE_MASK, [self])
	return not hit.is_empty()


# Smoothly moves the player up onto the target surface, spending stamina (cheaper and
# faster at higher Athletics) and briefly disabling collision so the move is clean.
func _execute_climb(destination: Vector3) -> void:
	is_climbing = true
	velocity = Vector3.ZERO

	var athletics_level: float = get_skill_level("athletics")
	var climb_time: float = 0.5 - (athletics_level / 100.0) * (0.5 - 0.3)
	var cost: float = CLIMB_STAMINA_COST - (athletics_level / 100.0) * (CLIMB_STAMINA_COST - 6.0)
	current_stamina = clamp(current_stamina - cost, 0.0, max_stamina)
	Helper.signal_broker.player_stamina_changed.emit(self, current_stamina)
	add_skill_xp("athletics", 0.5)

	# Disable collision during the climb so we can move through the obstacle cleanly.
	$CollisionShape3D.set_deferred("disabled", true)

	var tween: Tween = create_tween()
	(
		tween
		. tween_property(self, "global_position", destination, climb_time)
