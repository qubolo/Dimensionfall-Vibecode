...
func get_furniture_position_from_mapdata(furniture_data: Dictionary) -> Vector3:
	...
	# Otherwise, calculate the position from "global_position_x", "global_position_y", "global_position_z"
		...
		furniture_data.has("global_position_x")
		and furniture_data.has("global_position_y")
		and furniture_data.has("global_position_z")
	):
		return Vector3(
...
func _spawn_all_furniture():
	var count := 0
	# Spawn a small fixed number of furniture per frame. A fixed cap keeps the per-frame
	# spike bounded in dense areas (forests/cities); a batch proportional to the list size
	# spikes harder the denser the chunk, which caused stutter during chunk loading.
	var batch_size := 5
	...
	for furniture_data in furniture_json_list:
		...
		# Stagger spawning to prevent massive lag spikes in dense areas like forests and cities
		if count >= batch_size:
			count = 0
			await chunk.get_tree().process_frame
