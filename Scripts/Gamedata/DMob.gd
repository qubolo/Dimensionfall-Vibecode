...
# Undead hit-region model. When `undead` is true, this mob no longer dies from a
# single health pool. Instead it has separate pools: only depleting `brain_health`
# (the brain/spinal cord) kills it, while depleting `body_health` or `legs_health`
# leaves it crippled (a slow crawler). See Mob.get_hit for the resolution logic.
var undead: bool = false
var brain_health: int = 0  # Lethal pool. The head/spine. Reaching 0 means death.
var body_health: int = 0  # Torso/arm integrity. Reaching 0 cripples, never kills.
var legs_health: int = 0  # Mobility. Reaching 0 cripples (crawler), never kills.
var crawl_speed: float = 0.0  # Move speed while crippled. 0 means "derive from move_speed".
var parent: DMobs
...
func _init(data: Dictionary, myparent: DMobs):
	...
	# Undead hit-region properties (only meaningful when undead is true)
	undead = data.get("undead", false)
	brain_health = data.get("brain_health", 0)
	body_health = data.get("body_health", 0)
	legs_health = data.get("legs_health", 0)
	crawl_speed = data.get("crawl_speed", 0.0)
...
func get_data() -> Dictionary:
	...
	# Only write the undead block when this mob actually uses it, to keep
	# non-undead mob entries clean.
	if undead:
		data["undead"] = true
		data["brain_health"] = brain_health
		data["body_health"] = body_health
		data["legs_health"] = legs_health
		if crawl_speed > 0.0:
			data["crawl_speed"] = crawl_speed
	return data
...
func changed(olddata: DMob):
	...
	if old_loot_group and not old_loot_group == loot_group:
		...
		Gamedata.mods.remove_reference(
			DMod.ContentType.ITEMGROUPS, old_loot_group, DMod.ContentType.MOBS, id
		)
...
func delete():
	...
	var remove_from_quest: Callable = func(quest_id: String):
		...
		var allmodquests: Array = Gamedata.mods.get_all_content_by_id(
			DMod.ContentType.QUESTS, quest_id
		)
...
func update_mob_attack_references(olddata: DMob):
	...
	for old_attk_id in old_attk_ids:
		if not new_attk_ids.has(old_attk_id):
			Gamedata.mods.remove_reference(
				DMod.ContentType.ATTACKS, old_attk_id, DMod.ContentType.MOBS, id
			)

	# Add new attack references
	for new_attk_id in new_attk_ids:
		Gamedata.mods.add_reference(
			DMod.ContentType.ATTACKS, new_attk_id, DMod.ContentType.MOBS, id
		)
