...
class Magazine:
	...
	# Get data function to return a dictionary with all properties
	func get_data() -> Dictionary:
		return {"current_ammo": current_ammo, "max_ammo": max_ammo, "used_ammo": used_ammo}
...
class Melee:
	...
	var accuracy_stat: String
	# How the attack animates: "swing" (arc, e.g. axe/club) or "stab" (thrust, e.g. spear)
	var attack_motion: String
	...
	func _init(data: Dictionary):
		...
		accuracy_stat = data.get("accuracy_stat", "")
		attack_motion = data.get("attack_motion", "swing")

	# Get data function to return a dictionary with all properties
	func get_data() -> Dictionary:
		var data: Dictionary = {"damage": damage, "reach": reach, "used_skill": used_skill}
		if damage_stat != "":
			data["damage_stat"] = damage_stat
		if accuracy_stat != "":
			data["accuracy_stat"] = accuracy_stat
		# Only persist a non-default motion to keep weapon data tidy.
		if attack_motion != "swing":
			data["attack_motion"] = attack_motion
		return data
	...
	func add_reference(item_id: String):
		if slot != "":
			Gamedata.mods.add_reference(
				DMod.ContentType.WEARABLESLOTS, slot, DMod.ContentType.ITEMS, item_id
			)
...
func changed(olddata: DItem):
	...
	for res_id in old_resource_ids:
		if not new_resource_ids.has(res_id):
			Gamedata.mods.remove_reference(
				DMod.ContentType.ITEMS, res_id, DMod.ContentType.ITEMS, id
			)
...
func process_wearable_player_attributes(olddata: DItem):
	if not wearable:
		...
		if olddata.wearable and not olddata.wearable.player_attributes.is_empty():
			...
			for old_attr in olddata.wearable.player_attributes:
				Gamedata.mods.remove_reference(
					DMod.ContentType.PLAYERATTRIBUTES, old_attr["id"], DMod.ContentType.ITEMS, id
				)
	...
	for new_attr in new_player_attributes:
		...
		Gamedata.mods.add_reference(
			DMod.ContentType.PLAYERATTRIBUTES, attribute_id, DMod.ContentType.ITEMS, id
		)
	...
	for old_attr_id in old_attr_dict.keys():
		Gamedata.mods.remove_reference(
			DMod.ContentType.PLAYERATTRIBUTES, old_attr_id, DMod.ContentType.ITEMS, id
		)
...
func update_item_skill_references(olddata: DItem):
	...
	for old_skill_id in old_skill_ids:
		if not new_skill_ids.has(old_skill_id):
			Gamedata.mods.remove_reference(
				DMod.ContentType.SKILLS, old_skill_id, DMod.ContentType.ITEMS, id
			)

	# Add new skill references
	for new_skill_id in new_skill_ids:
		Gamedata.mods.add_reference(
			DMod.ContentType.SKILLS, new_skill_id, DMod.ContentType.ITEMS, id
		)
...
func update_item_stat_references(olddata: DItem) -> void:
	...
	for stat_id in old_stats:
		if not new_stats.has(stat_id):
			Gamedata.mods.remove_reference(
				DMod.ContentType.STATS, stat_id, DMod.ContentType.ITEMS, id
			)

	# 6) Add references for newly used stats
	for stat_id in new_stats:
		if not old_stats.has(stat_id):
			Gamedata.mods.add_reference(DMod.ContentType.STATS, stat_id, DMod.ContentType.ITEMS, id)
...
func update_item_attribute_references(olddata: DItem):
	...
	for new_attr_id in new_attr_ids:
		Gamedata.mods.add_reference(
			DMod.ContentType.PLAYERATTRIBUTES, new_attr_id, DMod.ContentType.ITEMS, id
		)
...
func delete():
	...
	if food and not food.attributes.is_empty():
		for food_attribute in food.attributes:
			Gamedata.mods.remove_reference(
				DMod.ContentType.PLAYERATTRIBUTES, food_attribute["id"], DMod.ContentType.ITEMS, id
			)
	...
	if wearable and not wearable.player_attributes.is_empty():
		for wearableattr in wearable.player_attributes:
			Gamedata.mods.remove_reference(
				DMod.ContentType.PLAYERATTRIBUTES, wearableattr["id"], DMod.ContentType.ITEMS, id
			)
	...
	if craft:
		...
		for resource in craft.get_all_used_items():
			Gamedata.mods.remove_reference(
				DMod.ContentType.ITEMS, resource, DMod.ContentType.ITEMS, id
			)
...
# Remove the reference of this item from each skill
	for skill_id in skill_ids.keys():
		Gamedata.mods.remove_reference(
			DMod.ContentType.SKILLS, skill_id, DMod.ContentType.ITEMS, id
		)
	...
	if craft:
		for recipe in craft.recipes:
			...
			if bonus_stat != "":
				Gamedata.mods.remove_reference(
					DMod.ContentType.STATS, bonus_stat, DMod.ContentType.ITEMS, id
				)

	if ranged and ranged.accuracy_stat != "":
		Gamedata.mods.remove_reference(
			DMod.ContentType.STATS, ranged.accuracy_stat, DMod.ContentType.ITEMS, id
		)
	if melee and melee.damage_stat != "":
		Gamedata.mods.remove_reference(
			DMod.ContentType.STATS, melee.damage_stat, DMod.ContentType.ITEMS, id
		)
	if melee and melee.accuracy_stat != "":
		Gamedata.mods.remove_reference(
			DMod.ContentType.STATS, melee.accuracy_stat, DMod.ContentType.ITEMS, id
		)
