...
func add_block_mobs():
	...
	for mobdata: Dictionary in mobdatalist:
		...
		if count >= batch_size:
			count = 0
			await get_tree().create_timer(0.2).timeout  # Stagger by 200 ms (adjust as needed)
...
func add_itemgroups_to_new_block():
	...
	# Spawn a small fixed number of itemgroups (loot containers) per frame. A fixed cap
	# keeps the per-frame spike bounded even in dense chunks (a proportional batch would
	# spike harder the more loot a chunk has).
	var delay_every_n_itemgroups = 5
	...
	for i in range(total_itemgroups):
		...
		if i % delay_every_n_itemgroups == 0 and i != 0:  # Avoid delay at the very start
			await get_tree().process_frame  # Adjust delay time as needed
...
func setup_navigation():
	...
	# cell_size drives how finely the navmesh is rasterized. 0.1 was very fine for a
	# 32-unit chunk, making each bake (and its main-thread apply) costly; 0.2 quarters
	# the cell count for a much cheaper bake with negligible pathing impact.
...
func create_colliders() -> void:
	...
	for key in block_positions.keys():
		...
		if block_shape == "slope":
			_add_collider_to_mesh_body.call_deferred(
...
func _create_combined_cube_collider(start_pos: Vector3, end_pos: Vector3) -> void:
	...
	_add_collider_to_mesh_body.call_deferred(collider)


# Adds a collider to the chunk's mesh body via a deferred call, guarding against the
# body having been freed in the meantime (e.g. the chunk was unloaded before the call
# ran). Without this guard, add_child would error on a "previously freed" object.
func _add_collider_to_mesh_body(collider: Node) -> void:
	if is_instance_valid(chunk_mesh_body):
		chunk_mesh_body.add_child(collider)
...
func add_block(block_id: String, block_position: Vector3):
	...
	_add_collider_to_mesh_body.call_deferred(new_collider)
