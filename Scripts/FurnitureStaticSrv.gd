...
# Static caches to avoid creating thousands of meshes for forests
...
func create_visual_instance(shape_type: String):
	...
	var cache_key = str(rfurniture.id) + "_" + shape_type
	if _mesh_cache.has(cache_key):
		support_mesh = _mesh_cache[cache_key]
	else:
		if shape_type == "Box":
		...
		_mesh_cache[cache_key] = support_mesh
...
func create_sprite_instance():
	...
	if _quad_mesh_cache.has(furnitureJSON.id):
		quad_mesh = _quad_mesh_cache[furnitureJSON.id]
	else:
		...
		_quad_mesh_cache[furnitureJSON.id] = quad_mesh
