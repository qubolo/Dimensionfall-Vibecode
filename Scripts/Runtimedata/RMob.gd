...
# Undead hit-region model (mirrors DMob). When `undead` is true the mob uses
# separate brain/body/legs pools instead of a single health pool: only brain
# damage is lethal, body/leg damage leaves it crippled. See Mob.get_hit.
var undead: bool = false
var brain_health: int = 0
var body_health: int = 0
var legs_health: int = 0
var crawl_speed: float = 0.0
...
func overwrite_from_dmob(dmob: DMob) -> void:
	...
	undead = dmob.undead
	brain_health = dmob.brain_health
	body_health = dmob.body_health
	legs_health = dmob.legs_health
	crawl_speed = dmob.crawl_speed
