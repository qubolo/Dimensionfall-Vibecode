...
# Undead hit-region model (zombies). When `undead` is true, this mob does not die
# from a single health pool. Instead damage is routed to one of three pools based
# on where the attacker aimed (see _resolve_hit_region):
# - brain: the brain/spinal cord. Depleting it is the ONLY way to kill the mob.
# - body: torso/arms. Depleting it cripples the mob (collapses to a crawler).
# - legs: mobility. Depleting it cripples the mob (collapses to a crawler).
# A crippled undead can still bite, and can only be finished off with a brain hit.
var undead: bool = false
var brain_health: float = 0.0
var current_brain_health: float = 0.0
var body_health: float = 0.0
var current_body_health: float = 0.0
var legs_health: float = 0.0
var current_legs_health: float = 0.0
var crawl_speed: float = 0.0  # Move speed once crippled
var is_crippled: bool = false  # True once body or legs have been destroyed
# How far (in meters, on the ground plane) the aim point must fall beyond the mob
# center to count as a head/spine shot, or short of it to count as a leg shot.
# Within this band around the center counts as a torso hit.
const HIT_REGION_BAND: float = 0.35
...
func _ready():
	...
	# Restore a saved undead's pools and crippled state (e.g. after loading a game)
	if undead:
		_restore_undead_state_from_json()
...
func get_hit(attack_data: Dictionary):
	...
	if randf() <= actual_hit_chance:  # / 100.0:
		# Attack hits
		if undead:
			# Undead route damage to a body region based on where it was aimed.
			# Only brain/spine damage is lethal; everything else cripples.
			_apply_undead_hit(damage, attack_data)
		else:
...
# Applies an incoming hit to an undead mob, routing the damage to the brain, body
# or legs depending on where the attacker aimed relative to this mob. Depleting the
# brain kills the mob; depleting the body or legs cripples it into a slow crawler.
func _apply_undead_hit(damage: float, attack_data: Dictionary) -> void:
	var region: String = _resolve_hit_region(attack_data)
	match region:
		"head":
			current_brain_health -= damage
			if current_brain_health <= 0:
				show_floating_label("Headshot!", Color(1, 0.9, 0.2))
				_die(last_attacker)
				return
		"legs":
			current_legs_health -= damage
			if current_legs_health <= 0 and not is_crippled:
				become_crippled()
		_:
			# Torso/arms (also the fallback when there is no aim data)
			current_body_health -= damage
			if current_body_health <= 0 and not is_crippled:
				become_crippled()
	if not is_blinking:
		start_blinking()


# Decides which body region an incoming attack struck, using the attacker's aim.
# In this top-down game the player aims with the cursor; aiming PAST/behind the mob
# strikes high (brain/spine), aiming SHORT/in front strikes low (legs) and aiming
# at the mob strikes the torso. Returns "head", "legs" or "torso". Without aim data
# (mob-vs-mob, saved data) it defaults to "torso" so the mob can never be killed by
# accident, matching the "only the brain is lethal" rule.
func _resolve_hit_region(attack_data: Dictionary) -> String:
	if not attack_data.has("aim_point"):
		return "torso"

	var aim_point: Vector3 = attack_data["aim_point"]
	var attacker_pos: Vector3 = attack_data.get("source_position", Vector3.ZERO)
	# Fall back to the live attacker position if no explicit source position was given
	if attacker_pos == Vector3.ZERO and last_attacker and is_instance_valid(last_attacker):
		attacker_pos = last_attacker.global_position

	# Everything is resolved on the ground (XZ) plane.
	var to_mob: Vector3 = global_position - attacker_pos
	to_mob.y = 0.0
	var dist_to_mob: float = to_mob.length()
	if dist_to_mob < 0.001:
		return "torso"

	var dir: Vector3 = to_mob / dist_to_mob
	var aim_vec: Vector3 = aim_point - attacker_pos
	aim_vec.y = 0.0
	# How far along the attacker -> mob line the aim point falls. Subtracting the
	# distance to the mob gives a signed depth: positive = aimed beyond the mob.
	var depth: float = aim_vec.dot(dir) - dist_to_mob

	if depth > HIT_REGION_BAND:
		return "head"
	if depth < -HIT_REGION_BAND:
		return "legs"
	return "torso"
...
# Turns the mob into a crippled crawler: it can no longer chase at full speed but
# remains dangerous up close. Only a brain/spine hit can finish it off afterwards.
func become_crippled() -> void:
	if is_crippled:
		return
	is_crippled = true
	current_move_speed = crawl_speed
	current_idle_move_speed = min(current_idle_move_speed, crawl_speed)
	show_floating_label("Crippled!", Color(1, 0.4, 0.1))


# Function to show a miss indicator
func show_miss_indicator():
	show_floating_label("Miss!", Color(1, 0, 0))


# Shows a short-lived floating label above the mob (used for "Miss!", "Headshot!"
# and "Crippled!" feedback). The label billboards toward the camera and fades out.
func show_floating_label(text: String, color: Color):
...
func apply_stats_from_dmob() -> void:
	...
	# Undead hit-region pools. Only used when the mob is flagged as undead.
	undead = rmob.undead
	if undead:
		brain_health = rmob.brain_health
		current_brain_health = rmob.brain_health
		body_health = rmob.body_health
		current_body_health = rmob.body_health
		legs_health = rmob.legs_health
		current_legs_health = rmob.legs_health
		# Default the crawl speed to a quarter of the normal speed when unset.
		crawl_speed = rmob.crawl_speed if rmob.crawl_speed > 0.0 else rmob.move_speed * 0.25
...
func get_data() -> Dictionary:
	...
	# Persist the undead pools and crippled state so a wounded zombie stays wounded
	# (and a crawler stays a crawler) after saving and loading.
	if undead:
		data["undead"] = true
		data["current_brain_health"] = current_brain_health
		data["current_body_health"] = current_body_health
		data["current_legs_health"] = current_legs_health
		data["is_crippled"] = is_crippled
	return data


# Restores a saved undead's current pools and crippled state from mob_json. Called
# from _ready so loading a game keeps wounded zombies wounded. Missing keys keep the
# full-health defaults already set by apply_stats_from_dmob.
func _restore_undead_state_from_json() -> void:
	if mob_json.has("current_brain_health"):
		current_brain_health = mob_json["current_brain_health"]
	if mob_json.has("current_body_health"):
		current_body_health = mob_json["current_body_health"]
	if mob_json.has("current_legs_health"):
		current_legs_health = mob_json["current_legs_health"]
	if mob_json.get("is_crippled", false):
		# Re-apply the crippled movement penalty without replaying the popup.
		is_crippled = true
		current_move_speed = crawl_speed
		current_idle_move_speed = min(current_idle_move_speed, crawl_speed)
