...
func test_mob_spawn():
	...
	# Mock pathfinding and simulate movement handling. The chunk's navigation mesh
	# bakes asynchronously, so it may not be ready the instant chunk_generated fires.
	# Retry making a path and moving until the mob actually produces a velocity.
	var mob_is_moving = func():
		mob_idle.makepath()
		mob_idle.handle_mob_movement()
		return first_mob.velocity.length() > 0.0
	assert_true(await wait_until(mob_is_moving, 10, 1), "Mob is not moving during idle movement.")
...
func test_mob_melee_combat():
	...
	# Mobs spawn staggered after chunk_generated fires, so wait until both are present.
	var two_mobs_present = func(): return get_tree().get_nodes_in_group("mobs").size() == 2
	assert_true(await wait_until(two_mobs_present, 5, 1), "too many or not enough mobs")
	var mobs: Array = get_tree().get_nodes_in_group("mobs")
	var first_mob: Mob = mobs[0]
	assert_eq(first_mob.rmob.id, "generic_test_mob", "A different mob spawned then expected")
